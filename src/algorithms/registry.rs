...
use super::sorting::{bubble_sort_by, insertion_sort_by, merge_sort_by, quick_sort};
...
impl AlgorithmRegistry {
    ...
    fn register_sorting_algorithms(&self) {
        self.register(
            "sort.bubble",
            BuiltinAlgorithm("sort.bubble", sort_bubble_handler),
        );
        self.register(
            "sort.insertion",
        ...
        self.register(
            "sort.merge",
            BuiltinAlgorithm("sort.merge", sort_merge_handler),
        );
        self.register(
            "sort.rust_default",
            BuiltinAlgorithm("sort.rust_default", sort_rust_default_handler),
        );
    }
    ...
    fn register_graph_algorithms(&self) {
        ...
        self.register(
            "graph.bfs",
            BuiltinAlgorithm("graph.bfs", graph_bfs_handler),
        );
        self.register(
            "graph.dfs",
            BuiltinAlgorithm("graph.dfs", graph_dfs_handler),
        );
        self.register(
    ...
    fn register_tree_algorithms(&self) {
        self.register(
            "tree.build",
            BuiltinAlgorithm("tree.build", tree_build_handler),
        );
        self.register(
            "tree.flatten",
            BuiltinAlgorithm("tree.flatten", tree_flatten_handler),
        );
    }
            ...
            match field {
                ...
                None => {
                    if reverse {
                        ...
                        bubble_sort_by(&mut data, json_ord);
                    }
                }
            }
            ...
            match field {
                ...
                None => {
                    if reverse {
                        ...
                        insertion_sort_by(&mut data, json_ord);
                    }
                }
            }
            AlgorithmOutput::new(serde_json::to_value(data).unwrap_or(serde_json::Value::Null))
            ...
            match field {
                ...
                None => {
                    if reverse {
                        ...
                        merge_sort_by(&mut data, json_ord);
                    }
                }
            }
            AlgorithmOutput::new(serde_json::to_value(data).unwrap_or(serde_json::Value::Null))
                ...
                while let Some(node) = queue.pop_front() {
                    ...
                    for n in neighbors {
                        if !visited.contains(&n)
                            && graph.nodes.iter().any(|node_obj| node_obj.id == n)
                        {
    ...
    match build_graph_from(&input.data) {
        Ok(graph) => {
            ...
            if let Some(start) = graph.nodes.first().map(|n| n.id.clone()) {
                fn dfs(graph: &Graph, node: &str, visited: &mut Vec<String>) {
...
macro_rules! define_algorithm {
    ($name:ident, $algo_name:expr, |$input:ident| $body:expr) => {
        ...
        impl $crate::algorithms::Algorithm for $name {
            fn execute(
                &self,
                $input: $crate::algorithms::AlgorithmInput,
            ) -> $crate::algorithms::AlgorithmOutput {
                $body
            }
...
mod tests {
    ...
    fn test_sort_bubble() {
        let reg = AlgorithmRegistry::new();
        let input = make_sort_input(serde_json::json!([5, 2, 8, 1, 9]), None, None);
    ...
    fn test_sort_with_field() {
        ...
        assert_eq!(
            arr[2].as_object().unwrap().get("name").unwrap(),
    ...
    fn test_sort_desc() {
        ...
        let input = make_sort_input(serde_json::json!([5, 2, 8, 1, 9]), None, Some("desc"));
        let out = reg.execute("sort.rust_default", input).unwrap();
        let arr = out.data.as_array().unwrap();
    ...
    fn test_validate_input_too_long() {
        let reg = AlgorithmRegistry::new();
        let input =
            AlgorithmInput::new(serde_json::json!({"text": "hello world", "max_length": 5}));
        let out = reg.execute("validate.input", input).unwrap();
        let valid = out.data.get("valid").and_then(|v| v.as_bool()).unwrap();
        assert!(!valid);
    }
    ...
    fn test_sanitize_escape_html() {
        let reg = AlgorithmRegistry::new();
        let input =
            AlgorithmInput::new(serde_json::json!({"text": "<script>alert('xss')</script>"}));
