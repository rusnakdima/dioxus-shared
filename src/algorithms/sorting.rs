...
mod tests {
    ...
    fn test_merge_sort_negative_numbers() {
        ...
        merge_sort(&mut v);
        assert_eq!(v, vec![-9, -4, -3, 1, 1, 2, 5, 6]);
    }
    ...
    #[test]
    fn test_bubble_sort_negative_numbers() {
        let mut v = vec![-5, 3, -1, 0, 7, -2];
        bubble_sort(&mut v);
        assert_eq!(v, vec![-5, -2, -1, 0, 3, 7]);
    }
    ...
    fn test_quick_sort_with_duplicates() {
        let mut v = vec![3, 1, 4, 1, 5, 1];
        quick_sort(&mut v);
        assert_eq!(v, vec![1, 1, 1, 3, 4, 5]);
    }

    proptest! {
      #[test]
      fn proptest_merge_sort_idempotent(v: Vec<i32>) {
        let mut arr1 = v.clone();
        let mut arr2 = v.clone();
        merge_sort(&mut arr1);
        merge_sort(&mut arr2);
      ...
      #[test]
      fn proptest_merge_sort_preserves_elements(v: Vec<i32>) {
        let mut arr = v.clone();
        let original_len = arr.len();
        let original_sum: i64 = arr.iter().map(|x| *x as i64).sum();
        merge_sort(&mut arr);
        prop_assert_eq!(arr.len(), original_len);
        let result_sum: i64 = arr.iter().map(|x| *x as i64).sum();
        prop_assert_eq!(result_sum, original_sum);
      }

      #[test]
      fn proptest_merge_sort_sorted_output(v: Vec<i32>) {
        let mut arr = v;
        merge_sort(&mut arr);
        for i in 1..arr.len() {
