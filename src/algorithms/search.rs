...
mod tests {
    ...
    proptest! {
      #[test]
      fn proptest_binary_search_index_in_bounds(sorted_vec: Vec<i32>, target: i32) {
        let mut arr = sorted_vec;
        prop_assume!(!arr.is_empty());
        arr.sort();
        let result = SearchAlgorithm::binary_search(&arr, &target);
        match result {
          Some(idx) => {
            prop_assert!(idx < arr.len(), "index {} out of bounds for len {}", idx, arr.len());
            prop_assert_eq!(arr[idx], target);
          }
          None => {
            let exists = arr.iter().any(|x| *x == target);
            prop_assert!(!exists, "binary_search returned None but element {} exists in array", target);
          }
        }
      }

      #[test]
      fn proptest_binary_search_correct_position(sorted_vec: Vec<i32>, target: i32) {
        let mut arr = sorted_vec;
        prop_assume!(!arr.is_empty());
        arr.sort();
        arr.dedup();
